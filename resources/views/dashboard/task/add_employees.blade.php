<div class="container mt-5">
    ...
    @endif
    <h2>Add Employees to Task</h2>
    <p><strong>ID:</strong> {{ $task->id }}</p>
    <p><strong>Name:</strong> {{ $task->name }}</p>

    <!-- Search Form -->
    <form method="GET" action="{{ route('task.addEmployees', $id) }}">
        <div class="card p-3 mb-3">
            ...
            <div class="d-flex justify-content-between mt-2">
                ...
                <a href="{{ route('task.addEmployees', $id) }}" class="btn btn-secondary btn-sm">Reset</a>
            </div>
        </div>
    </form>
    ...
    <form method="POST" action="{{ route('task.addEmployees', $task->id) }}">
        @csrf
                ...
                @foreach ($employees as $employee)
                    <tr>
                        <td><input class="employee-ids" id="employee-id-{{ $employee->id }}"
                                onclick="addToTask({{ $employee->id }}, {{  $task->id }})" type="checkbox"
        ...
        <div class="d-flex justify-content-between mt-2">
            ...
            <button type="button" onclick="clearSessionStorage({{ $task->id }})" class="btn btn-secondary btn-sm">Clear
                changes</button>
        </div>
    </form>
</div>
<script>
    function clearSessionStorage(taskId) {
        let itemName = 'Task-id-' + taskId;
        ...
        precheckEmployeeBelongTask(taskId);
    }
    ...
    function precheckEmployeeBelongTask(taskId) {
        let itemName = 'Task-id-' + taskId;
    ...
    precheckEmployeeBelongTask({{ $task->id }});
</script>